...
class Events_Calendar_Shortcode
{
				...
				$output .= '<li class="ecs-event">';
					...
					if( static::isValid($ecs_thumb) ) {
						...
						if( !empty($thumbWidth) && !empty($thumbHeight) ) {
							$output .= get_the_post_thumbnail($post_id, array($thumbWidth, $thumbHeight) );
						} else {
							$output .= get_the_post_thumbnail($post_id, 'medium');
						}
					}

					if( static::isValid($ecs_excerpt) ) {
	 ...
	 * @return boolean
	 */
	private static function isValid( $prop ) 
	{
		return ($prop !== 'false');
	}
